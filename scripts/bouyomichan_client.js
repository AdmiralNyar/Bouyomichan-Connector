 ...
 * 読み上げる
 */
BouyomiChanClient.prototype.talk = async function (cmntStr, voice, volum) {
    this.cmntStr = cmntStr;
    this.voicesetting = voice;
    this.volum = volum;
    //'ws://' + HOST + ':' + PORT + '/ws/'
...
BouyomiChanClient.prototype.socket_onopen = function (e) {
    ...
    // 棒読みちゃんデータを生成
    let data = this.makeBouyomiChanDataToSend(this.cmntStr, this.voicesetting, this.volum);
 ...
 * 棒読みちゃんへ送信するデータの生成
 */
BouyomiChanClient.prototype.makeBouyomiChanDataToSend = function (cmntStr, voicesetting, volum) {
    ...
    let volume = volum; //[6-7]  (16Bit) 音量              （-1:棒読みちゃん画面上の設定）
    let voice = voicesetting; //[8-9]  (16Bit) 声質              （ 0:棒読みちゃん画面上の設定、1:女性1、2:女性2、3:男性1、4:男性2、5:中性、6:ロボット、7:機械1、8:機械2、10001～:SAPI5）
